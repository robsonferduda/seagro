<div id="myCarousel" class="carousel slide" data-bs-ride="carousel">
    <!-- Carousel indicators -->
    <ol class="carousel-indicators">
        <li data-bs-target="#myCarousel" data-bs-slide-to="0" class=""></li>
        <li data-bs-target="#myCarousel" data-bs-slide-to="1" class=""></li>
        <li data-bs-target="#myCarousel" data-bs-slide-to="2" class="active" aria-current="true"></li>
    </ol>
    
    <!-- Wrapper for carousel items -->
    <div class="carousel-inner">
        @for($i = 0; $i < 3; $i++)
            <div class="carousel-item carousel-principal {{ ($i == 0) ? "active" : "" }}">
                <a href="{{ url('noticia', $noticias[$i]['url']) }}"><img src="{{ $noticias[$i]->urlCapa() }}" class="d-block w-100" alt="{{ $noticias[$i]['titulo'] }}"></a>
                <div class="carousel-caption d-none d-md-block">
                    <h5></h5>
                    <p>{{ $noticias[$i]['titulo'] }}</p>
                </div>
            </div>
        @endfor        
    </div>

    <!-- Carousel controls -->
    <a class="carousel-control-prev" href="#myCarousel" data-bs-slide="prev">
        <span class="carousel-control-prev-icon"></span>
    </a>
    <a class="carousel-control-next" href="#myCarousel" data-bs-slide="next">
        <span class="carousel-control-next-icon"></span>
    </a>
</div>